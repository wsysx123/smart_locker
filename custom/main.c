...
#define MQTT_TIMER_ID 		TIMER_ID_USER_START + 2
#define KEEPLIVE_TIMER_ID		TIMER_ID_USER_START + 3


#define TCP_TIMER_PERIOD     800
#define TIMEOUT_90S_PERIOD   90000

#define MQTT_TIMER_PERIOD     2000
#define TIMEOUT_KEEPLIVE_PERIOD   100000
...
u8 m_send_buf[SEND_BUFFER_LEN];
u8 m_recv_buf[RECV_BUFFER_LEN];
static u64 m_nSentLen  = 0;      // Bytes of number sent data through current socket    

static u8  m_SrvADDR[SRVADDR_BUFFER_LEN] = "119.29.187.104";
...
s32 m_remain_len = 0;     // record the remaining number of bytes in send buffer.
char *m_pCurrentPos = NULL; 
...
void proc_main_task(s32 taskId)
{
	...
	Ql_Timer_Register(MQTT_TIMER_ID, Callback_MQTT_Timer, NULL);
	Ql_Timer_Start(MQTT_TIMER_ID, MQTT_TIMER_PERIOD, TRUE);

	Ql_Timer_Register(KEEPLIVE_TIMER_ID, Callback_MQTT_Timer, NULL);
	...
	Ql_GPIO_Init(PINNAME_SIM2_CLK,PINDIRECTION_OUT,PINLEVEL_LOW,PINPULLSEL_PULLUP);
	//Ql_GPIO_SetLevel(PINNAME_SIM2_CLK,PINLEVEL_LOW);
	Ql_GPIO_Init(PINNAME_PCM_SYNC,PINDIRECTION_IN,PINLEVEL_HIGH,PINPULLSEL_PULLDOWN);
	Ql_GPIO_Init(PINNAME_PCM_CLK,PINDIRECTION_IN,PINLEVEL_HIGH,PINPULLSEL_PULLDOWN);
	extinit();
...
void callback_socket_read(s32 socketId, s32 errCode, void* customParam )
{
    ...
    do
    {
        ...
        else if(ret < RECV_BUFFER_LEN)
        {
			...
			Ql_memcpy(ackmsg,m_recv_buf,ret);
			acklen = ret;
            break;
        }else if(ret == RECV_BUFFER_LEN)
        {
...
u8 devlink(){
	...
	//	send2server("\1\2\3",3);
		...
		RIL_GetIMEI(strtmp);
		Ql_memcpy(strtmp,strtmp+2,15);
		strtmp[15]=0;
		Ql_memcpy(strtmp+15, "|securemode=3,signmethod=hmacsha1|",34);
		//Ql_strcat(strtmp,"|securemode=3,signmethod=hmacsha1|");
		APP_DEBUG("devicename: %s  \r\n",mqttusername);
		APP_DEBUG("username: %s  \r\n",strtmp);
		APP_DEBUG("password: %s  \r\n",mqttpassword);
	if(MQTT_PacketConnect(mqttusername,mqttpassword,strtmp,120, 0, MQTT_QOS_LEVEL0, NULL, NULL, 0, &mqttbuf)==0){
		APP_DEBUG( "PacketConnect ok! len:%d\r\n",mqttbuf._len);
		//Ql_UART_Write(UART_PORT1,mqttbuf._data,mqttbuf._len);
		send2server(mqttbuf._data,mqttbuf._len);
		APP_DEBUG( "sending to server\r\n");
...
static void Callback_MQTT_Timer(u32 timerId, void* param){
	if (MQTT_TIMER_ID == timerId){
		switch (MQTTSTATE){
			case STATE_MQTT_TCP_CONNECT:
				devlink();
				//send2server("",102);
				
				MQTTSTATE = STATE_MQTT_MQTT_CONNECT_WAITCONNECTACK;
			
				break;
			case STATE_MQTT_MQTT_CONNECT_WAITCONNECTACK:
				MQTT_RevPro(ackmsg);
				mqttwaitingack++;
				if(mqttwaitingack>=5){
					MQTTSTATE = STATE_MQTT_TCP_CONNECT;
				...
				break;
			case STATE_MQTT_MQTT_CONNECT_OK:
				APP_DEBUG("status==connect ok");
				MQTT_Subscribe(topics,1);
				MQTTSTATE = STATE_MQTT_MQTT_SUBSCRIBE_WAITCONNECTACK;
				Ql_Timer_Start(KEEPLIVE_TIMER_ID, TIMEOUT_KEEPLIVE_PERIOD, TRUE);
				break;
			case STATE_MQTT_MQTT_SUBSCRIBE_WAITCONNECTACK:
				...
				break;
			case STATE_MQTT_MQTT_SUBSCRIBE_OK:
				MQTTSTATE = STATE_MQTT_MQTT_WAITING;
				break;
			case STATE_MQTT_MQTT_PUBLISH_PACKET:
				packet(packetlen,packetdata);
				break;
			case STATE_MQTT_MQTT_PUBLISH:
				MQTT_Publish(topics[0],publish,publishlen);
				//MQTTSTATE = STATE_MQTT_MQTT_PUBLISH_WAITACK;//若QOS！=0
				MQTTSTATE =STATE_MQTT_MQTT_PUBLISH_OK;
				break;			
			case STATE_MQTT_MQTT_PUBLISH_WAITACK:
				MQTT_RevPro(ackmsg);
				mqttwaitingack++;
				if(mqttwaitingack>=5){
					MQTT_Publish(topics[0],lastpublish,lastpublishlen);
					mqttwaitingack=0;
					}
				break;
			case STATE_MQTT_MQTT_PUBLISH_OK:
				MQTTSTATE = STATE_MQTT_MQTT_WAITING;
				break;
			case STATE_MQTT_MQTT_WAITING:
				MQTT_RevPro(ackmsg);
				break;
			default:
				break;
	...
	/*		second++;
		...
		else if(KEEPLIVE_TIMER_ID==timerId){
			 APP_DEBUG("<--KEEPLIVE time out!!!-->\r\n");
   		 		MQTT_Ping();
			}
...
/*void send2server(char * data,u32 len){
	...
	for(int i=0;i<m_remain_len;i++){
		APP_DEBUG("%d:%x ",i, m_send_buf[i]);
		}
	APP_DEBUG("\r\n");
