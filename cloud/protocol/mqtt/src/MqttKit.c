...
{
	mqttPacket->_data = dataMem;				//鍏ㄥ眬鏁扮粍鏂瑰紡
	mqttPacket->_len = 0;
	mqttPacket->_size = MQTTSIZE;		//鍏ㄥ眬鏁扮粍鏂瑰紡
...
uint8 MQTT_UnPacketRecv(uint8 *dataPtr)
{
	...
	if(type == MQTT_PKT_PUBLISH)
	{
		...
		if(Ql_strstr((int8 *)msgPtr + 2, CMD_TOPIC_PREFIX) != NULL)	//如果是命令下发
			status = MQTT_PKT_CMD;
		else
...
uint8 MQTT_UnPacketCmd(uint8 *rev_data, int8 **cmdid, int8 **req, uint16 *req_len)
{
	...
	int8 *dataPtr = Ql_strchr((int8 *)rev_data + 3, '/');	//加3是跳过头信息
	...
	*cmdid = (int8 *)MQTT_MallocBuffer(37);				//cmdid固定36字节，多分配一个结束符的位置
	...
	Ql_memset(*cmdid, 0, 37);								//全部清零
	Ql_memcpy(*cmdid, (const int8 *)dataPtr, 36);			//复制cmdid
	dataPtr += 36;
	...
	*req = (int8 *)MQTT_MallocBuffer(*req_len + 1);		//分配命令长度+1
	...
	Ql_memset(*req, 0, *req_len + 1);						//清零
	Ql_memcpy(*req, (const int8 *)dataPtr, *req_len);		//复制命令
	...
	switch(*qos)
	{
		case MQTT_QOS_LEVEL0:									// qos0 have no packet identifier
			...
			*topic = MQTT_MallocBuffer(*topic_len + 1);			//为topic分配内存
			...
			Ql_memcpy(*topic, (int8 *)msgPtr + 2, *topic_len);		//复制数据
